...
model = sys.argv[1]
page_path = os.path.join(DATA_FOLDER, sys.argv[2])
xml_path =  os.path.join(DATA_FOLDER, sys.argv[3])
if sys.argv[4] != '':
    model_file =  os.path.join(MODEL_FOLDER, sys.argv[4])
else:
    model_file = ''
if sys.argv[5] != '':
    pretrain_word_file =  os.path.join(DATA_FOLDER, sys.argv[5])
else:
    pretrain_word_file = ''
...
try:
    ...
    tmp_dir = tempfile.mkdtemp(dir='/AI/tmp')
    ...
    # save model
    new_model_file = tempfile.mktemp(dir=MODEL_FOLDER) + '.h5'
    model.save_weights(new_model_file)
    print(os.path.basename(new_model_file))
    ...
    loss_plot_file = tempfile.mktemp(dir=DATA_FOLDER) + ".png"
    plt.savefig(loss_plot_file)
    print(os.path.basename(loss_plot_file))

    classes_data_file = tempfile.mktemp(dir=DATA_FOLDER) + ".json"
    # turn all nparray to json
    with open(classes_data_file, 'w') as outfile:
        json.dump(classes_data, outfile)
    print(os.path.basename(classes_data_file))
